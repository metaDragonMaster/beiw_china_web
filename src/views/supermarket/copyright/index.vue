<template>
	<div class="view ">
		<div class="banner limit-max-width-inset">
			...
			<h1 class="logo-title">版权超市</h1>
			<el-form ref="Form" :model="Form" label-position="top" inline class="flex-center">
				<el-form-item prop="keyWord" label="版权名称"><el-input v-model="Form.keyWord"  placeholder="请输入关键字" @blur="startGetTable()" clearable></el-input></el-form-item>
				<el-form-item prop="type" label="版权类型">
					<el-input v-model="Form.type"  placeholder="请输入版权类型" @blur="startGetTable()" clearable></el-input>
				...
				<el-form-item prop="createSort" label="版权排序">
		...
		<ul class="goods-list">
			<li v-for="(item, index) in tableData" :key="index">
				...
				<div class="info">
					...
					<div class="flex-center"><el-button type="success" @click="toDetails(item.id)">查看版权</el-button></div>
				</div>
			</li>
		</ul>
...
export default {
	...
	methods: {
		...
		toDetails(id) {
			this.$routerUtil.toPath('/supermarket/copyright/details', { id: id });
		},
		async getTableData() {
			let params = {
				...this.Form
			...
			let { code, data, msg } = await Api.selectCopyRightInfoList(params);
			if (code == 10200) {
