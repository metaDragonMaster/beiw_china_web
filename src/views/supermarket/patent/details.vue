...
export default {
	...
	mounted() {
		...
		this.setInsertPatentInfoById(id);
	},
	methods: {
		async setInsertPatentInfoById(id) {
			let { code, data, msg } = await Api.setInsertPatentInfoById(id);
			if (code == 10200) {
...
<template>
	<div class="view supermarket-patent-details limit-max-width">
		...
		<div class="inquiry flex-column-center-js-sb">
			...
			<el-button type="primary" :style="{ width: 'fit-content' }" @click="$routerUtil.toPath('/supermarket/patent')">返回超市</el-button>
		</div>
		<div class="content">
			<el-tabs v-model="activeName" type="card">
				<el-tab-pane label="商品详情" name="first">
					...
					<el-descriptions class="descriptions" :column="3" size="medium" border>
						<el-descriptions-item label="专利(申请号)">{{info.number}}</el-descriptions-item>
						...
						<el-descriptions-item label="专利类型">{{info.type}}</el-descriptions-item>
				...
				<el-tab-pane label="产品咨询" name="third">
					<p class="head-title">资产评估报告（专利）</p>
					...
					<p>
						...
						<span>1、委托方、资产持有方：A公司，A公司于5年前自行研发了该项大功率电热转换体及其处理技术，并获得了发明专利证书，专利的保护期为20年。</span>
						<span>2、评估报告其他使用方：企业专利技术转让相对应的机构及法律、法规另有规定的使用方。</span>
					</p>
					<p>
						<span>二、评估目的</span>
						<span>
					...
					<p>
						...
						<span>“预期收益价值”，在此被定义为，该专利技术所生产的产品在未来销售过程中为企业带来的利润收入现值。</span>
					</p>
					<p>
						<span>五、评估基准日</span>
						<span>
				...
				<el-tab-pane label="价值评估报告" name="fourth">
					<el-descriptions class="descriptions" :column="2" v-for="(item, index) in 10" :key="index">
						...
						<el-descriptions-item label="资讯内容" :span="2" :contentStyle="{ color: '#46A0E3' }">请问这个专利的价格可以优惠吗？</el-descriptions-item>
