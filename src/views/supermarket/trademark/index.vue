<template>
	<div class="view ">
		<div class="banner limit-max-width-inset">
			...
			<h1 class="logo-title">商标超市</h1>
			<el-form ref="Form" :model="Form" label-position="top" inline class="flex-center">
				<el-form-item prop="keyWord" label="商标名称"><el-input v-model="Form.keyWord"  placeholder="请输入关键字" @blur="startGetTable()"></el-input></el-form-item>
				<el-form-item prop="type" label="商标类型">
					<el-input v-model="Form.type"  placeholder="请输入商标类型" @blur="startGetTable()"></el-input>
					<!-- <el-select v-model="Form.type" placeholder="请选择"  @change="startGetTable()" clearable>
						<el-option v-for="item in selectPatentType" :key="item.value" :label="item.label" :value="item.value"></el-option>
					</el-select> -->
				</el-form-item>
				<el-form-item prop="createSort" label="商标排序" >
					<el-select v-model="Form.createSort" placeholder="请选择"  @change="startGetTable()" clearable>
		...
		<ul class="goods-list ">
			<li v-for="(item, index) in tableData" :key="index">
				...
				<div class="info">
					...
					<div class="flex-center"><el-button type="success" @click="toDetails(item.id)">查看商标</el-button></div>
				</div>
			</li>
		</ul>
...
<script>
import Api from '@/api/tradeMark.js';
...
export default {
	...
	methods: {
		...
		toDetails(id) {
			this.$routerUtil.toPath('/supermarket/trademark/details', { id: id });
		},
		async getTableData() {
			let params = {
				...this.Form
			...
			let { code, data, msg } = await Api.selectTradeMarkInfoList(params);
			if (code == 10200) {
